...
app.get('/api/support', verifyToken, verifyAdmin, async (req, res) => {
  try {
...
// Reply to support ticket via email (Admin only)
app.post('/api/support/reply', verifyToken, verifyAdmin, async (req, res) => {
  try {
...
app.patch('/api/support/:id/status', verifyToken, verifyAdmin, async (req, res) => {
  try {
    const { id } = req.params;
